...
class OrderService {
  ...
  static async viewOrders() {
    const res = await DatabaseMethods.query({
      query:
        "SELECT o.idorder, o.client, o.total, o.status, o.comments, MONTH(o.date) mes FROM `order` AS o WHERE o.active=1 ORDER BY o.date ASC",
      params: [],
    });
    return res;
  }
  ...
  static async misOrdenes(idUser: string){
    const res = await DatabaseMethods.query({
      query:
        "SELECT o.idorder, u.name AS client, o.total, o.status, o.comments, o.date FROM `order` AS o JOIN users u ON u.idusers = o.users_idusers WHERE o.users_idusers = ? AND o.active = 1 ORDER BY o.date DESC",
      params: [idUser],
    });
    return res;
  }
